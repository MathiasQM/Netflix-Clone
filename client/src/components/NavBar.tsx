...
const tabs = [, "Series", "Films", "New & Popular", "Browse by Languages"];
    ...
    <nav className="w-full fixed z-40 top-0">
      <div className={`px-16 flex items-center ${showBg ? "bg-black/80" : ""}`}>
        ...
        <div className="flex gap-7 ml-8 mr-auto">
          ...
          {tabs.map((tab) => (
            <div key={tab} className="text-white whitespace-nowrap hover:text-gray-500 cursor-pointer">
              <p>{tab}</p>
            </div>
          ))}
        </div>
        {user && !isLoading && (
          <div className="flex gap-5">
            <div className="text-white  whitespace-nowrap hover:text-gray-500 cursor-pointer ml-auto">
